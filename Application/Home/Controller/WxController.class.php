    ...
    public function checkKeyword($key)
    {
        ...
        if ($key == "推广二维码") {
            ...
            if (!$vip) {
                ...
                $msg = "您还未成为" . self::$_shop['fxname'] . "，请先进入会员中心 >> 成为" . self::$_shop['fxname'] . "！";
                self::$_wx->text($msg)->reply();
                exit();
            }
...
//            \Util\QRcode::png($url, './QRcode/qrcode/' . $openid . '.png', 'L', 6, 2);
    ...
    public function woo($key){
        
        file_put_contents('./Data/app_rev.txt', '131收到请求:' . date('Y-m-d H:i:s') . PHP_EOL . '通知信息:' . $key . PHP_EOL . PHP_EOL . PHP_EOL, FILE_APPEND);
    }
} //API类结束