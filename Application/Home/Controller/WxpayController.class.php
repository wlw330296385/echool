...
class WxpayController extends Controller
{
    ...
    public function pay()
    {
        ...
        $paysdk->setParameter("attach", $attach); //交易类型
...
//		$_SESSION['tmpwxpay']=$wOpt;
    ...
    //充值成功
    private function cz($oid){
        $vlDB = M('vip_log');
        $w['id'] = $oid;
        $w['type'] = 7;
        $w['status'] = 0;
        $vl = $vlDB->where($w)->find();
        if($vl){
            //开始给会员加钱
            $vip = M('Vip')->where('id=' . $vl['vipid'])->setInc('money',$vl['money']);
            $vlDB->where($w)->setField('status',1);
        }
    }
    //支付成功后后台接受方案
