...
class AlipayController extends Controller
{
    ...
    {
        $alipay_config = M("Alipay")->where(array('id'=>2))->find();
        $config = array(
            // 即时到账方式
            'payment_type' => 1,
            // 传输协议
...
//        }
        ...
        if ($order) {
            if ($order['status']) {
                ...
                if (FALSE !== $re) {
                    ...
                    $templateid = 'yBsZw8MHAkjVGnFOMfYG5lKMWT1GgRXBYqGnFSjl4CE';
                    $mvip = M('Vip');
                    ...
                    if ($templateid) { // 存在才可以发送模板消息
                        ...
                        $wxset= D('Set')->find();
                        // 调试
                        // file_put_contents("./Data/appid.txt",$wxset['wxappid'] . PHP_EOL .$wxset['wxappsecret']. PHP_EOL .json_encode($data),FILE_APPEND);
                        $options['appid'] = $wxset['wxappid'];
                        $options['appsecret'] = $wxset['wxappsecret'];

                        $wx = new \Util\Wx\Wechat($options);
                        $aa = $wx->sendTemplateMessage($data);
                        //发送商家模板消息
                        $this->sendMobanMsmToShop($order['oid'],1);
                        //打印小票
    ...
    {
        //构造消息体
        $order = M('shop_order')->where(array('oid'=>$oid))->find();
        // $shop = M('Shop')->where('id=' . $order['sid'])->find();

        $vipid = $order['vipid'];
        
            if ($order['vipmobile'] == '') {
                $addressinfo = M('vip_address')->where(array('vipid'=>$vipid))->order('id DESC')->select();
                if ($addressinfo) {
                ...
                $customerInfo =  $order['vipmobile'] . '收货地址:' .$order['vipaddress'];
            }
            $first = '新订单通知（已付款）';
            ...
            $data['data'] = array(
                ...
                'keyword1' => array('value' => $order['oid']),
                'keyword2' => array('value' => $order['paytype']),
                'keyword3' => array('value' => $order['payprice']  ),
            ...
            foreach ($wxid as $k => $val) {
                ...
                file_put_contents('./msg.txt', PHP_EOL . '发送商家模板消息成功:' . date('Y-m-d H:i:s') . $rtn, FILE_APPEND);
            }
    }
