...
class VipController extends BaseController
{
    ...
    public function index()
    {
        ...
        //将字符串转换为表情
        $data['nickname']=preg_replace("#\\\u([0-9a-f]+)#ie","iconv('UCS-2','UTF-8', pack('H4', '\\1'))",$data['nickname']);
        ...
        if ($re) {
            switch ($type) {
                case '1':
                    ...
                    break;
                case '2':
                    $this->redirect('Home/Wxpay/pay', array('price' => $money, 'oid' => $re,'type'=>'cz'));
                    break;
                default:
                    $this->error('支付方式未知！');
                    break;
...
//        $map['type'] = 2; //充值卡充值
    ...
    public function toBangshou()
    {
        ...
        $is_a = $m->where($cond)->getField('id');
        if ($is_a) {
            $saveCond = array(
                'id'=>$is_a
            );
            $m->where($saveCond)->setField('aptime', time());
            $msg['res'] = '已再次提交申请,请耐心等待';
        } else {
            $m->add($data);
            $msg['res'] = 1;
        }
    ...
    public function test(){
        ...
        $re=$alipay->endpay('20160621112918-996');
        // dump($alipay);
        // dump($_SESSION);
    }
}
