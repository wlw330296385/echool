...
class VipController extends BaseController
{
    ...
    public function set()
    {
        ...
        if (IS_POST) {
            ...
            file_put_contents('./Data/app_debug.txt','日志时间:'.date('Y-m-d H:i:s').PHP_EOL.'纪录信息:'.json_encode($data).PHP_EOL.PHP_EOL.PHP_EOL.M('vip_set')->getLastSql().PHP_EOL.'金额'.PHP_EOL.PHP_EOL,FILE_APPEND);
    ...
    public function vipTrack()
    {
        ...
        if (IS_POST) {
            ...
            if (count($cache) > 0) {
                ...
                //将字符串转换为表情
                     $vip['nickname']=preg_replace("#\\\u([0-9a-f]+)#ie","iconv('UCS-2','UTF-8', pack('H4', '\\1'))",$vip['nickname']);
                    if ($vip['type'] == 1) {
        ...
        foreach ($top as $key => $value) {
             $top[$key]['nickname']=preg_replace("#\\\u([0-9a-f]+)#ie","iconv('UCS-2','UTF-8', pack('H4', '\\1'))",$top[$key]['nickname']);
        }
    ...
    public function vipSetfx(){
        ...
        if ($id) {
            $isfx=I('get.isfx');
            if ($isfx==1) {
               $res=$vip->where('id='.$id)->setField('isfx',1);
            }
            M('vip_apply')->where('userid='.$id)->delete();
            $msg['res']=1;
        }  
        die(json_encode($msg));
    }
    ...
    //设置面包导航，主加载器请配置
        ...
        foreach ($arr as $key => $value) {
           $cache[$key]=$vip->where('id='.$value['userid'])->find(); 
        }
        $this->assign('cache',$cache);
        $this->display();
    }
    ...
    public function cardToAll(){
        ...
        $bread = array(
            ...
            '2' => array(
                'name' => '一件发送设置',
                'url' => U('Admin/Vip/cardToAll'),
            ),
        );
        $this->assign('breadhtml', $this->getBread($bread));
         // 插入卡券成功模板消息=====================
        $templateidshort = 'OPENTM202240584';
        $dwechat = D('Wechat');
        ...
        if (IS_POST) {
            ...
            unset($data['num']);
            //一键生成并发送卡券
            $vipList=M('Vip')->select();
            // $vipList=M('Vip')->where("`id`=2162 OR `id`=1915")->select();
            foreach ($vipList as $key => $value) {
                $cardnopwd = $this->getCardNoPwd();
                $data['cardno'] = $cardnopwd['no'];
                $data['cardpwd'] = $cardnopwd['pwd'];
    ...
    //             $templateid = $dwechat->getTemplateId($templateidshort);  
    //             //发送模板消息
    //             $one=M('vip_card')->where(array('openid'=>$openid,'status'=>1))->find();
    //             if ($one) {
    //                 $msg = array();
    //                 $msg['touser'] = $openid;
    //                 $msg['template_id'] = $templateid;
    //                 $msg['topcolor'] = "#00FF00";
    //                 $msg['url'] = "http://www.gxeschool.com/App/Vip/card";
    //                 $msg['data'] = array(
    //                         'first'    => array('value' => '别忘了您的背包里有张卡券哦'),
    //                         'keyword1' => array('value' =>  $one['cardno']),
    //                         'keyword2' => array('value' => '满'.$usemoney.'使用'),
    //                         'keyword3' => array('value' => 'Eschool校园平台'),
    //                         'keyword4' => array('value' =>1),
    //                         'remark'   => array('value' => '请您保管好卡券号，并在有效期内使用,赶快告诉别人吧(｡･∀･)ﾉﾞ [点击可查看]')
    //                     );
    //                 $options['appid'] = self::$SYS['set']['wxappid'];
    //                 $options['appsecret'] = self::$SYS['set']['wxappsecret'];
    //                 $wx = new \Util\Wx\Wechat($options);
    //                 $remsg = $wx->sendTemplateMessage($msg);//发送微信消息 
    //                 // 发送模板消息结束=================
    //             }
    //         if ($remsg) {
    //             $info['msg']='1'; 
    ...
    public function getOpenid(){
        $vipList=M('Vip')->select();
            foreach ($vipList as $key => $value) {
               $arr[$key]=$value['openid'];
            }
        die(json_encode($arr));
    }
    private function msgToall(){
                set_time_limit(0);
                $viplist=M('Vip')->select();
                $openidarr=array();
                foreach ($viplist as $k => $val) {
                        $openidarr[$k]=$val['openid'];
                    }    
                // 插入卡券发送成功模板消息=====================
                $templateidshort='OPENTM202240584';
                $dwechat = D('Wechat');
                ...
                //发送模板消息
                foreach ($viplist as $key => $value) {
                    $data=M('vip_card')->where(array('openid'=>$value['id'],'status'=>1))->find();
                    $msg = array();
                    $msg['touser'] = $value['openid'];
                    $msg['template_id'] = $templateid;
                    $msg['topcolor'] = "#00FF00";
                    $msg['url'] = "http://www.gxeschool.com/App/Vip/card";
                    $msg['data'] = array(
                            'first'    => array('value' => '别忘了您的背包里有张卡券哦'),
                            'keyword1' => array('value' =>  $data['cardno']),
                            'keyword2' => array('value' => '满'.$data['usemoney'].'使用'),
                            'keyword3' => array('value' => 'Eschool校园平台'),
                            'keyword4' => array('value' => date("Y-m-d h:i:sa", data('ctime'))),
                            'remark'   => array('value' => '请您保管好卡券号，并在有效期内使用,赶快告诉别人吧(｡･∀･)ﾉﾞ [点击可查看]')
                        );
                    $options['appid'] = self::$SYS['set']['wxappid'];
                    $options['appsecret'] = self::$SYS['set']['wxappsecret'];
    ...
    public function sendCard()
    {
        ...
        $vip=M('vip')->where('id=' . $post['vipid'])->find();
        ...
        $data['status'] = 1;
        $result=$m->where('id=' . $post['cardid'])->find();
        ...
        if ($re) {
            ...
            $info['msg'] = '发送成功!';
             // // 插入卡券发送成功模板消息=====================
                $templateidshort = 'OPENTM202240584';
                $dwechat = D('Wechat');
                $templateid ='EwPix8x8YHW9_YKwGnmKL-PPJdwp_m9Vg63HhqPc2uo';
                //发送模板消息
                if ($templateid) { // 存在才可以发送模板消息
                    ...
                    $msg['url'] = "http://www.gxeschool.com/App/Vip/card";
                    $msg['data'] = array(
                            'first'    => array('value' => '您好，您获得一张优惠券'),
                            'keyword1' => array('value' => $result['cardno']),
                            'keyword2' => array('value' => '满'.$result['usemoney'].'使用'),
                            'keyword3' => array('value' => 'Eschool校园平台'),
                            'keyword4' => array('value' => date("Y-m-d h:i:sa", time())),
                            'remark'   => array('value' => '请您保管好卡券号，并在有效期内使用,赶快告诉别人吧(｡･∀･)ﾉﾞ[点击可查看]')
                        );
                    $options['appid'] = self::$SYS['set']['wxappid'];
                    $options['appsecret'] = self::$SYS['set']['wxappsecret'];
    ...
    public function courierSet(){
        ...
        $bread = array(
            ...
            '1' => array(
                'name' => '配送员列表',
                'url' => U('Admin/Vip/courierList'),
            ),
            '1' => array(
                'name' => '配送员编辑',
                'url' => U('Admin/Vip/courierSet', array('id' => $id)),
            ),
        );
        $this->assign('breadhtml', $this->getBread($bread));
        //处理POST提交
    ...
    public function emoji($text){
        //将字符串转换为表情
       $text = preg_replace("#\\\u([0-9a-f]+)#ie","iconv('UCS-2','UTF-8', pack('H4', '\\1'))",$text);
       return $text;
    }
