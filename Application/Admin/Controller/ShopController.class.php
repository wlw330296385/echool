...
class ShopController extends BaseController
{
    ...
    public function goods()
    {
        ...
        $cache = $m->where($map)->page($p, $psize)->order('sorts asc')->select();
        $count = $m->where($map)->count();
        // $num=M('Shop_goods_sku')->where('goodsid=4')->select();
        // $sum=0;
    ...
    public function goodsSet()
    {
        ...
        if (IS_POST) {
            ...
            $data['content'] = trimUE($data['content']);
                ...
                $data['shortsells'] = strtotime($data['shortsells'])*1000;
                $data['shortselle'] = strtotime($data['shortselle'])*1000;                                      
            if ($id) {
                if ($data['issells']==0) {
                    $one=$m->where('id='.$id)->find();
                    $data['dissells']=$one['sells'];
                }
                $re = $m->save($data);
                if (FALSE !== $re) {
                    $info['status'] = 1;
    ...
    public function order()
    {
        ...
        //绑定筛选时间
        $parameter=I('path.');
        $actime = $_GET['actime'] ? $_GET['actime'] :'';
        $usetime = I('usetime')?I('usetime'):'';
        if ($actime != '') {
            $timeArr = explode(" - ", $actime);
        ...
        $summoney = $m->where($map)->page($p, $psize)->order('ctime desc')->sum('payprice');
        $sum= $m->where($map)->sum('payprice');
        $count = $m->where($map)->count();
        if (strlen($parameter[3])==23) {
            $this->getPage($count, $psize, 'App-loader', '商城订单', 'App-search',array('actime'=>$parameter[3],'status'=>$status));
        }else{
            $this->getPage($count, $psize, 'App-loader', '商城订单', 'App-search',array('actime'=>$parameter[3],'status'=>$status));
        } 
        $sum=round($sum,2);
    ...
    public function orderFhkd()
    {
        ...
        $this->assign('cache', $cache);
        $mb = $this->fetch();//解析模板(跟display差不多)的内容
        $this->ajaxReturn($mb);
    }
    //选择配送员
    public function orderToCourier(){
        $map['id'] = I('id');
        $courier=M('courier')->select();
        $cache = M('Shop_order')->where($map)->find();
        $this->assign('cache',$cache);
    ...
    public function orderToSave(){
        ...
        $data['fahuokdnum'] = $data['fahuokd'].'0000'.time();
        $courier=$data['fahuokd'];
        $data['fahuokd'] = M('courier')->where(array('id'=>$data['fahuokd']))->getField('name');
        $re = M('Shop_order')->where('id=' . $data['id'])->save($data);
        if (FALSE !== $re) {            
            //发送模板消息
            $courier=M('courier')->where(array('id' => $courier))->find();
            $order = M('Shop_order')->where('id=' . $data['id'])->find();
            $order['ispay']=$order['ispay']=='1'?'已付款':'未付款';
             // 插入订单发货模板消息=====================
                    $vip = M('vip')->where(array('id' => $courier['vipid']))->find();
                    $templateidshort = 'OPENTM205504305';
                    ...
                    if ($templateid) { // 存在才可以发送模板消息
                        ...
                        $msg['topcolor'] = "#D4EDFB";
                        $msg['url'] = "http://www.gxeschool.com/App/Shop/orderList_wx/oid/".$order['oid'];
                        $msg['data'] = array(
                            'first' => array('value' => '您好，有一件商品需要发货'),
                            'keyword1' => array('value' => date('Y-m-d H:m:s',$order['paytime'])),
                            'keyword2' => array('value' => $order['vipname'].'/'.$order['vipmobile'].$order['vipaddress']),
                            'keyword3' => array('value' => $order['oid']),
                            'keyword4' => array('value' => $order['totalprice']),
                            'keyword5' =>array('value'=>$order['paytype'].'['.$order['ispay'].']'),
                            'remark' => array('value' => '商品总数:'.$order['totalnum'].';备注:'.$order['msg'])
                        );
                         $options['appid'] = self::$SYS['set']['wxappid'];
                        $options['appsecret'] = self::$SYS['set']['wxappsecret'];
                        $wx = new \Util\Wx\Wechat($options);
                        $sendMSG = $wx->sendTemplateMessage($msg);
                        if (FALSE !== $sendMSG) {
                            $info['status'] = 1;
                            $info['msg'] = '操作成功,已向配送员发送通知！'; 
                        }
                    }
                    // 插入订单发货模板消息结束=================
        } else {
    ...
    public function orderTuihuoSave()
    {
        ...
        switch ($cache['status']) {
            case '4':
                ...
                $re = $m->where('id=' . $data['id'])->save($data);//修改退货状态

                if (FALSE !== $re) {
                    $vip['money'] = $vip['money'] + $data['tuihuoprice'];//把钱返回给用户
                    $rvip = $mvip->save($vip);
                    if ($rvip !== FALSE) {
                        //前端LOG
                        $log['oid'] = $cache['id'];
                        ...
                        //修改库存
                        $cache['items'] = unserialize($cache['items']);
                        foreach ($cache['items'] as $key => $value) {
                           M('Shop_goods')->where('id='.$value['goodsid'])->setInc('num',$value['num']);//修改商品库存
                           M('Shop_goods')->where('id='.$value['goodsid'])->setDec('sells',$value['num']);//修改已售
                           if ($value['skuid']) {
                              M('Shop_goods_sku')->where('id='.$value['skuid'])->setInc('num',$value['num']);//修改sku库存
                           }  
                        }
                    } else {
                        //前端LOG
                        ...
                        //    M('Shop_goods')->where('id='.$value['goodsid'])->setInc('num',$value['num']);//修改商品库存
                        //    if ($value['sku']) {
                        //       M('Shop_goods_sku')->where('sku='.$value['sku'])->setInc('num',$value['num']);//修改sku库存
                        //    }  
                        // }
