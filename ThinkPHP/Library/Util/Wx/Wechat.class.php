...
<Nonce><![CDATA[%s]]></Nonce>
    ...
    public function sendTemplateMessage($data)
    {
        ...
        file_put_contents('./Data/woo.txt', PHP_EOL . 'WECHAT:' .json_encode($result). date('Y-m-d H:i:s'), FILE_APPEND);
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
...
class ErrorCode
{
    ...
    //type=0:订单新生成（未付款）
    //type=1:订单已付款
    public function sendMobanMsmToShop($oid, $wxid, $flag = FALSE)
    {
        //构造消息体
        ...
        $ppid = $order['vipid'];
            ...
            $data['url'] = "http://www.gxeschool.com/App/Shop/orderList_wx";
            $data['data'] = array(
